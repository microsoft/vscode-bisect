...
class Launcher {
    ...
    private async runLinuxDesktopInstaller(quality: Quality, flavor: Flavor.LinuxDeb | Flavor.LinuxRPM | Flavor.LinuxSnap, path: string): Promise<IInstance | undefined> {
        ...
        switch (flavor) {
            case Flavor.LinuxDeb:
                installCommand = `sudo dpkg -r ${quality === Quality.Stable ? 'code' : 'code-insiders'} && sudo dpkg -i ${path}`;
                executeCommand = quality === Quality.Stable ? 'code' : 'code-insiders';
                break;
            case Flavor.LinuxRPM:
                installCommand = `sudo rpm -e ${quality === Quality.Stable ? 'code' : 'code-insiders'} && sudo rpm -i ${path}`;
                executeCommand = quality === Quality.Stable ? 'code' : 'code-insiders';
                break;
            case Flavor.LinuxSnap:
                installCommand = `sudo snap remove ${quality === Quality.Stable ? 'code' : 'code-insiders'} && sudo snap install ${path} --classic --dangerous`;
                executeCommand = 'snap';
                executeArgs = ['run', quality === Quality.Stable ? 'code' : 'code-insiders'];
                break;
        }
    ...
    private getWindowsVSCodeExecutablePath(flavor: Flavor.WindowsUserInstaller | Flavor.WindowsSystemInstaller, quality: Quality): string {
        ...
        const isInsiders = quality !== Quality.Stable;
    ...
    private async launchDockerCLI(build: IBuild, flavor: Flavor.CliLinuxAmd64 | Flavor.CliLinuxArm64 | Flavor.CliLinuxArmv7 | Flavor.CliAlpineAmd64 | Flavor.CliAlpineArm64): Promise<IInstance> {
        ...
        const quality = build.quality === Quality.Stable ? 'stable' : 'insider';
