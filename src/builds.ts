...
class Builds {
    ...
    async fetchBuildByVersion({ runtime, quality, flavor }: IBuildKind, version: string): Promise<IBuild> {
        ...
        if (quality === Quality.Stable) {
            ...
            // For both insider and exploration, use the insider API (exploration may need specific handling in the future)
            const qualityStr = quality === Quality.Exploration ? 'exploration' : 'insider';
            meta = await jsonGet<IBuildMetadata>(`https://update.code.visualstudio.com/api/versions/${version}.0-${qualityStr}/${this.getBuildApiName({ runtime, quality, flavor })}/${qualityStr}?released=true`);
        }
    ...
    private async getBuildName({ runtime, commit, quality, flavor }: IBuild): Promise<string> {
        ...
        if (flavor !== Flavor.Cli) {
            switch (platform) {
                case Platform.MacOSX64:
                case Platform.MacOSArm:
                    return quality === Quality.Stable ? 'Visual Studio Code.app' : 'Visual Studio Code - Insiders.app';
                case Platform.LinuxX64:
                case Platform.LinuxArm:
        ...
        // CLI
        return quality === Quality.Stable ? 'code' : 'code-insiders';
    }
    ...
    async getBuildExecutable({ runtime, commit, quality, flavor }: IBuild): Promise<string> {
        ...
        if (runtime === Runtime.WebLocal || runtime === Runtime.WebRemote) {
            switch (platform) {
                ...
                case Platform.LinuxArm: {
                    ...
                    return join(buildPath, buildName, 'bin', quality === Quality.Stable ? 'code-server' : 'code-server-insiders');
                }
                case Platform.WindowsX64:
                case Platform.WindowsArm: {
                    ...
                    return join(buildPath, buildName, buildName, 'bin', quality === Quality.Stable ? 'code-server.cmd' : 'code-server-insiders.cmd');
                }
            }
        }
        ...
        if (flavor !== Flavor.Cli) {
            switch (platform) {
                ...
                case Platform.LinuxX64:
                case Platform.LinuxArm:
                    return join(buildPath, buildName, quality === Quality.Stable ? 'code' : 'code-insiders')
                case Platform.WindowsX64:
                case Platform.WindowsArm:
                    return join(buildPath, buildName, quality === Quality.Stable ? 'Code.exe' : 'Code - Insiders.exe');
            }
        }
